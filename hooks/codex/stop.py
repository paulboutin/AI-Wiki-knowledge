"""
Stop hook for Codex - captures conversation transcript for memory extraction.

When a Codex session ends (Stop event), this hook reads the transcript path from
stdin, extracts conversation context, and spawns flush.py as a background process
...
def main() -> None:
    ...
    logging.info("Codex Stop fired: session=%s", session_id)
    ...
    # Set env var to prevent recursive hook firing
    env = os.environ.copy()
